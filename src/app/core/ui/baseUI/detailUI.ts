...
export abstract class DetailUI<TMaster extends BaseEntity, TChild extends BaseEntity> implements OnInit {
    ...
    onSeek(): void {
        ...
        let r = this.collection(this.masterInstance.id, this.filterInstance);
        //console.log('result : ', lastValueFrom(r));
    ...
    onEditButtonClick(modal: IEditUI<TChild>, currentRecord?: TChild): void {
        if (currentRecord)
    ...
    onRowDblClick(modal: IEditUI<TChild>, mouseEvent?: MouseEvent): void {
        if (mouseEvent && mouseEvent.ctrlKey)
            this.onEditButtonClick(modal);
        else {
            let info = this.currentInstance.info;
            console.log('info : ', info);
            this.router.navigate([`${info.url}`, this.currentInstance.id]);
        }
    }
}