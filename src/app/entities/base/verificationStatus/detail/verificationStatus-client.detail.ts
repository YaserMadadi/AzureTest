
import { Component, inject, Input } from '@angular/core';
import { CommonModule } from '@angular/common';
import { FormsModule } from '@angular/forms';

import { DetailUI } from '../../../../../core/ui/baseUI';
import { SHARED_PIPES } from '../../../../../core/ui/pipes/sharedPipes';
import { LookupComponent, RowButtons, DetailButton } from '../../../../../core/ui/components';
import { ForeignkeyLinker } from '../../../../../core/ui/helper';


import { VerificationStatus, VerificationStatus_ServiceCollection } from '../../verificationStatus';
import { Client, Client_Builder } from '../../client';
import { ClientEditUI } from '../../client/edit/client.edit'
import { ClientDeleteUI } from '../../client/delete/client.delete'
import { City, City_Service } from '../../city';
import { Gender, Gender_Service } from '../../gender';
import { UserAccount, UserAccount_Service } from '../../userAccount';



@Component({
  selector: 'base-verificationStatus-client-detail',
  templateUrl: './verificationStatus-client.detail.html',
  styleUrls: ['./verificationStatus-client.detail.scss'],
  imports: [
    FormsModule,
    CommonModule,
    SHARED_PIPES,
    DetailButton,
    RowButtons,
    LookupComponent,
    ClientEditUI,        
    ClientDeleteUI,
  ]
}) 
export class VerificationStatus_Client_DetailUI extends DetailUI<VerificationStatus, Client> {

  constructor(private serviceCollection: VerificationStatus_ServiceCollection = inject(VerificationStatus_ServiceCollection)) {
    super(serviceCollection.CollectionOfClient_PhoneVerificationStatus.bind(serviceCollection), inject(Client_Builder));
    this.currentInstance = new Client();
  }

  @Input()
  public set verificationStatus(value: VerificationStatus) {
    this.masterInstance = { ...value };
    //this.currentInstance.verificationStatus = { ...value };
    this.sourceInstance.phoneVerificationStatus = { ...value };
  }

  public cityLinker: ForeignkeyLinker<City> = new ForeignkeyLinker<City>(inject(City_Service), true);

		public genderLinker: ForeignkeyLinker<Gender> = new ForeignkeyLinker<Gender>(inject(Gender_Service), true);

		public userAccountLinker: ForeignkeyLinker<UserAccount> = new ForeignkeyLinker<UserAccount>(inject(UserAccount_Service), true);

		

}
