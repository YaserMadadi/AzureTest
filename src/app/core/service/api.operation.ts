...
export class API_Operation<T extends BaseEntity> extends EndPointController {
  ...
  Seek(entity: T): Observable<ResultData<T[]>> {
    console.log('FilterInstance 2 : ', entity);
  ...
  CollectionOf<U extends BaseEntity>(sourcEntity: T, entity: U, extendedPath: string = ''): Observable<ResultData<U[]>> {
    ...
    let url = `${environment.baseUrl}/${sourcEntity.info.schema}/${extendedPath}/${sourcEntity.id}/${entity.info.name}`;
    return this.http
      .post<ResultData<U[]>>(url, entity, EndPointController.Options);
    // return new Observable<ResultData<U[]>>();
  }

  LoadFactory<U extends BaseEntity>(entity: T, info: Info): Observable<U> {
    let url = `${environment.baseUrl}/${entity.info.schema}/${entity.info.name}/${entity.id}/${info.name}`;
    return this.http.get<U>(url, EndPointController.Options);
  }

  GetByUrl<T1 extends BaseEntity>(url: string): Observable<ResultData<T1[]>> {
    url = environment.baseUrl + url;
    console.log('Url : ', url);
    return this.http.get<ResultData<T1[]>>(url, EndPointController.Options);
  }
