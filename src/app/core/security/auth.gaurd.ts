...
import { UserType } from "../../entities/admin/userType";
...
export class AuthGuard implements CanActivate {
    ...
    private static _userAccount: UserAccount = new UserAccount();
    public static set UserAccount(value: UserAccount) {
        AuthGuard._userAccount = value;
        StorageController.Set("UserAccount", JSON.stringify(value));
    }

    public static get UserAccount(): UserAccount {
        if (AuthGuard._userAccount == null || AuthGuard._userAccount.id <= 0)
            AuthGuard._userAccount = JSON.parse(StorageController.Get("UserAccount") ?? '{}') as UserAccount;
        return AuthGuard._userAccount;
    }
    ...
    public static Clear() {
        ...
        AuthGuard.UserAccount = new UserAccount();
